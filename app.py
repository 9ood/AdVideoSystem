...
def process_video(task_id):
    try:
        video_filename, video_path = get_task_video(task_id)
        if not video_path:
        ...
        missing_keyframes = [
            frame_info["scene_number"]
            for frame_info in frames_info
            if not frame_info.get("keyframe_path") or not os.path.exists(frame_info["keyframe_path"])
        ]
...
def download_result(task_id):
    ...
    try:
        ...
        return jsonify({"error": f"生成 ZIP 文件失败: {str(e)}"}), 500
...
def download_clean_result(task_id):
    ...
    try:
        ...
        return jsonify({"error": f"生成纯净版 Excel 失败: {str(e)}"}), 500
